...
@section('konten')
    <section class="mb-6 rounded-[2.2rem] kartu p-6 sm:p-8">
        ...
        <h2 class="mt-2 text-3xl font-black text-slate-950">Pilih layanan, lokasi, dan detail AC.</h2>
        <p class="mt-2 text-sm leading-7 text-slate-600">
            Merk AC wajib dipilih untuk layanan Beli AC, Jual AC, dan Service AC.
        </p>
    </section>
    ...
    <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        @foreach($layanan as $item)
            <form action="{{ route('pesanan.store') }}" method="POST" class="kartu rounded-[2rem] p-6 transition duration-200 hover:-translate-y-1 hover:shadow-2xl">
                ...
                <textarea name="catatan" rows="3" class="textarea-form mt-2" placeholder="Contoh: AC 1 PK, kamar depan, unit outdoor sulit dijangkau."></textarea>
